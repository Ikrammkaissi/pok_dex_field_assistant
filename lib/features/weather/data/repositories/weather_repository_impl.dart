...
///
/// Lives in the data layer — the only layer allowed to import HTTP clients.
/// The domain layer depends on the abstract [WeatherRepository] interface, not
/// this class, so either API can be swapped without touching use cases or the controller.
...
class WeatherRepositoryImpl implements WeatherRepository {
  ...
  /// Returns a parsed [WeatherData] with temperature, windspeed, weathercode,
  /// and the derived [WeatherData.suggestedPokemonType].
  @override
  ...
  Future<List<PokemonSummary>> getPokemonByType(String typeName) async {
    ...
    try {
      ...
      /// Map each entry to a [PokemonSummary] without extra network calls.
      final summaries = pokemonList
          ...
          .whereType<PokemonSummary>() // filter out nulls from skipped entries
          .toList();
